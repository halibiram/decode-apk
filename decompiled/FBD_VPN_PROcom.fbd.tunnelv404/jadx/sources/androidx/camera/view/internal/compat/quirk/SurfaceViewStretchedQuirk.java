...
public class SurfaceViewStretchedQuirk implements Quirk {
    ...
    private static boolean isLenovoTablet() {
        if (LENOVO.equalsIgnoreCase(Build.MANUFACTURER) && LENOVO_TAB_P12_PRO.equalsIgnoreCase(Build.DEVICE)) {
            return true;
        }
        return false;
    }
    ...
    private static boolean isOppoFoldable() {
        if (OPPO.equalsIgnoreCase(Build.MANUFACTURER) && OPPO_FIND_N.equalsIgnoreCase(Build.DEVICE)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean load() {
        if (Build.VERSION.SDK_INT < 33 && (isSamsungFold2OrFold3() || isOppoFoldable() || isLenovoTablet())) {
            return true;
        }
        return false;
    }
