...
public final class LargeJpegImageQuirk implements Quirk {
    ...
    private static boolean isSamsungProblematicDevice() {
        if ("Samsung".equalsIgnoreCase(Build.BRAND) && SAMSUNG_DEVICE_MODELS.contains(Build.MODEL.toUpperCase(Locale.US))) {
            return true;
        }
        return false;
    }
    ...
    private static boolean isVivoProblematicDevice() {
        if ("Vivo".equalsIgnoreCase(Build.BRAND) && VIVO_DEVICE_MODELS.contains(Build.MODEL.toUpperCase(Locale.US))) {
            return true;
        }
        return false;
    }
    ...
    public static boolean load() {
        if (!isSamsungDevice() && !isVivoProblematicDevice()) {
            return false;
        }
        return true;
    }
