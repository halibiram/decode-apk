...
public class VideoEncoderConfigDefaultResolver implements Supplier<VideoEncoderConfig> {
    ...
    private static final Size VIDEO_SIZE_BASE = new Size(VpnProfile.DEFAULT_MSSFIX_SIZE, 720);
    ...
    private int resolveFrameRate() {
        ...
        if (!Objects.equals(range, range2)) {
            i = VALID_FRAME_RATE_RANGE.clamp(this.mExpectedFrameRateRange.getUpper()).intValue();
        } else {
            i = 30;
        }
        ...
        Logger.d(TAG, String.format("Default resolved frame rate: %dfps. [Expected operating range: %s]", valueOf, obj));
        return i;
    }
