...
public final class Cam16 {
    ...
    public static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        double hypot = (Math.hypot(d11, d12) * (viewingConditions.getNcb() * (viewingConditions.getNc() * (((Math.cos(Math.toRadians(d4) + 2.0d) + 3.8d) * 0.25d) * 3846.153846153846d)))) / (d14 + 0.305d);
        ...
        return new Cam16(d16, sqrt, pow4, flRoot, flRoot2, sqrt2, d18, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
    ...
    public double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] dArr) {
        ...
        double d2 = (((0.305d + nbb) * 23.0d) * pow) / (((pow * 108.0d) * sin) + (((11.0d * pow) * cos2) + (ncb * 23.0d)));
        double d3 = cos2 * d2;
        double d4 = d2 * sin;
        ...
        double pow4 = Math.pow(Math.max(0.0d, (Math.abs(d7) * 27.13d) / (400.0d - Math.abs(d7))), 2.380952380952381d) * (100.0d / viewingConditions.getFl()) * Math.signum(d7);
        double pow5 = Math.pow(Math.max(0.0d, (Math.abs(d8) * 27.13d) / (400.0d - Math.abs(d8))), 2.380952380952381d) * (100.0d / viewingConditions.getFl()) * Math.signum(d8);
        double d9 = pow3 / viewingConditions.getRgbD()[0];
        double d10 = pow4 / viewingConditions.getRgbD()[1];
        double d11 = pow5 / viewingConditions.getRgbD()[2];
